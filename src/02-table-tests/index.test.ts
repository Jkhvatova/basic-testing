// Uncomment the code below and write your tests
import { simpleCalculator, Action } from './index';

const testCases = [
  { a: 1, b: 2, action: Action.Add, expected: 3 },
  { a: 2, b: 2, action: Action.Add, expected: 4 },
  { a: 3, b: 2, action: Action.Add, expected: 5 },
  { a: 4, b: 3, action: Action.Subtract, expected: 1 },
  { a: 5, b: 7, action: Action.Subtract, expected: -2 },
  { a: 6, b: 2, action: Action.Subtract, expected: 4 },
  { a: 1, b: 2, action: Action.Multiply, expected: 2 },
  { a: 2, b: 2, action: Action.Multiply, expected: 4 },
  { a: 3, b: 4, action: Action.Multiply, expected: 12 },
  { a: 10, b: 2, action: Action.Divide, expected: 5 },
  { a: 2, b: 2, action: Action.Divide, expected: 1 },
  { a: 6, b: 3, action: Action.Divide, expected: 2 },
  { a: 1, b: 2, action: Action.Exponentiate, expected: 1 },
  { a: 2, b: 2, action: Action.Exponentiate, expected: 4 },
  { a: 4, b: 3, action: Action.Exponentiate, expected: 64 },
  { a: 'invalidArgument', b: 3, action: Action.Add, expected: null },
  { a: 4, b: 'invalidArgument', action: Action.Multiply, expected: null },
  {
    a: 'invalidArgument',
    b: 'invalidArgument',
    action: Action.Divide,
    expected: null,
  },
  { a: 4, b: 3, action: 'Action.Invalid', expected: null },
  { a: 4, b: 'invalidArgument', action: 'Action.Invalid', expected: null },
];

describe('simpleCalculator', () => {
  test.each(testCases)(
    'should perform $action operation on $a and $b and return $expected',
    ({ a, b, action, expected }) => {
      const result = simpleCalculator({ a, b, action });
      expect(result).toBe(expected);
    },
  );
});
